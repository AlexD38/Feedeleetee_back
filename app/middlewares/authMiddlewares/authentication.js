...
const authMiddleware = {
	...
	createToken(req, res, next) {
		...
		if (!token) {
			...
			const enterpriseId =
				res.locals.user.enterprise_id || res.locals.user.enterpriseId;
			const clientId =
				res.locals.user.client_id || res.locals.user.clientId;
			const userId = res.locals.user.id | res.locals.user.userId;
			const userName =
				res.locals.user.user_name || res.locals.user.userName;
	...
	async verifyUser(req, res, next) {
		...
		try {
			...
			const hashedPwdFromDb = mailFound.password;
			...
			const hasMatched = bcrypt.compareSync(password, hashedPwdFromDb);
			if (!hasMatched) {
				res.json({ error: "...Mauvais identifiant" });
				return;
			}
			const userFound = mailFound;
