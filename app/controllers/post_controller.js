...
const post_controller = {
	...
	async attachEnterpriseToUser(req, res, next) {
		try {
			...
			const enterpriseIdFromEnterpriseCreated =
			...
			if (userId) {
				...
				const fetchUserWithEnterprise = await get_model.getOneUser(
					userId
				);
			} else {
				res.json({
	...
	async uploadLogo(req, res) {
		try {
			...
			const enterpriseId =
				res.locals.user.enterpriseId || req.locals.user.enterprise_id;
			// console.log(imageData);
