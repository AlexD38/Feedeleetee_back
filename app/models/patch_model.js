...
const patch_model = {
	...
	//je passe en argument, ce que j'extrair de req.body
	async updateEnterprise(name, address, description, enterpriseId) {
		// getion d'erreur avec try / catch
		try {
			console.log(name, address, description);
			const sqlQuery = {
				text: `UPDATE enterprises SET name = $1, address=$2,description= $3 WHERE id = $4`,
				values: [name, address, description, enterpriseId],
			};
			// j'execute la query
			await client.query(sqlQuery);
			// je range mon client dans la variable client created
			...
			// je renvoie le résultat
			const result = {
				success: ` ${enterpriseId} has been successfully updated`,
			};
			return result;
		} catch (error) {
			console.error(error);
